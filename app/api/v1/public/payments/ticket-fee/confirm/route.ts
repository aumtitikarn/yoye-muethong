...
import { finalizeTicketFeeCharge } from "@/lib/booking-finalize";

export const runtime = "nodejs";
export const dynamic = "force-dynamic";

// POST /api/v1/public/payments/ticket-fee/confirm  { chargeId }
// Called by the client once polling sees the ค่าบัตร charge as paid, so the
// booking is settled even if the Omise webhook isn't reachable (e.g. local).
// Safe: the charge is re-verified against Omise inside finalizeTicketFeeCharge,
...
export async function POST(req: NextRequest) {
  ...
  try {
    const result = await finalizeTicketFeeCharge(chargeId);
    if (!result.ok) {
    ...
    console.error("ticket-fee confirm error:", err);
    return NextResponse.json(
