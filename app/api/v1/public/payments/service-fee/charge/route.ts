...
// POST /api/v1/public/payments/service-fee/charge  { nonce, bookingCode }
// Charges the ค่ากด for an existing FORM booking that reached FORM_HAS_NAME.
// The amount is (re)computed server-side from feePerEntry × entries — never
// trusting the client — and stashed in the charge metadata so the webhook /
// confirm can settle the booking. The booking itself is advanced to COMPLETED
// only once the charge is confirmed paid (see finalizeServiceFeeCharge).
export async function POST(req: NextRequest) {
  const ip = clientIpFromRequest(req);
  const rl = consumeRateLimit(
    `service-fee-charge:${ip}`,
    RATE_LIMIT_MAX,
  ...
  // Ownership is enforced here (loadOwnedBooking returns null without a valid
  // LINE session or when the booking isn't the caller's).
  ...
  const info = serviceFeeInfo(booking);
  if (!info.payable) {
    return NextResponse.json(
      { message: "รายการนี้ยังไม่อยู่ในสถานะที่ต้องชำระค่ากด" },
      { status: 409 },
    );
  }
  ...
  if (amountSatang <= 0) {
    return NextResponse.json({ message: "ยอดค่ากดไม่ถูกต้อง" }, { status: 400 });
  }
  ...
  try {
    ...
    const charge = await createCharge({
      nonce,
      amount: amountSatang,
      returnUri: `${origin}/payments/${encodeURIComponent(bookingCode)}?omise_return=1`,
      metadata: {
        bookingCode,
        kind: "service_fee",
        serviceFeeBaht: String(info.amountBaht),
      },
    });
    return NextResponse.json({ data: { ...charge, bookingCode } });
  } catch (err) {
    console.error("service-fee charge error:", err);
    return NextResponse.json(
      {
        message:
