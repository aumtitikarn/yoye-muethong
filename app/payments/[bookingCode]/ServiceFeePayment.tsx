...
import {
  createServiceFeeCharge,
  confirmServiceFeeCharge,
...
// PromptPay is the default; everything else (except installments) is offered as
// an alternative. Methods not enabled on the Opn account simply won't appear.
const OTHER_METHODS = [
  "credit_card",
  "internet_banking",
  "mobile_banking",
...
const PENDING_KEY = "yoye_omise_pending_service_fee";
...
export default function ServiceFeePayment({
  ...
  const markSuccess = useCallback(
    async (chargeId: string) => {
      ...
      try {
        await confirmServiceFeeCharge(chargeId);
      } catch (err) {
        console.error("confirm service fee failed:", err);
      }
      setPhase({ kind: "success" });
      toast.success("ชำระค่ากดสำเร็จ");
  ...
  const chargeMutation = useMutation({
    mutationFn: (nonce: string) =>
      createServiceFeeCharge({ nonce, bookingCode }),
  ...
  const openCheckout = useCallback(() => {
    ...
    if (amountSatang <= 0) {
      toast.error("ยอดค่ากดไม่ถูกต้อง");
      return;
    }
    ...
    <div className="space-y-3">
      ...
      {phase.kind === "success" ? (
        <div className="rounded-xl border border-emerald-200 bg-emerald-50 p-4 flex items-center gap-2 text-emerald-700 font-semibold">
          <CheckCircle2 className="size-5" /> ชำระค่ากดสำเร็จ
        ...
        <>
          ...
          >
            {phase.kind === "processing" ? (
              ...
              <>
                <CreditCard className="size-4" /> ชำระค่ากด ฿
